...
class CrudCassandra:
    ...
    async def get():
        ...
        query = "SELECT * FROM appeals WHERE name=%s"
        future = conn.execute_async(query, ["test_name_2"])
        ...
        rows = future.result()
        appeal = rows[0]
        print(appeal.id, appeal.name, appeal.value)
        return {"msg": "appeal.id, appeal.name, appeal.value"}
        # except ReadTimeout:
...
#     @staticmethod
#     async def set(company_name, first_name, last_name, user_email, user_pass):
...
#                 ''', first_name, last_name, user_email, hashed_password, is_admin)
#                 user_id = user_row_id["id"]
#                 user_is_admin = user_row_id["is_admin"]

#                 # Связываем пользователя и компанию
#                 await conn.execute('''
#                     INSERT INTO join_users_companies (user_id, company_id)
#                     VALUES ($1, $2);
#                 ''', user_id, company_id)
...
#                     'SELECT company_id FROM join_users_companies WHERE user_id = $1',
#                     user_id
#                 )
